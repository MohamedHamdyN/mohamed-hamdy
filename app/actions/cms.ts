...
// ============= FREELANCE PLATFORMS CRUD =============

export async function getFreelancePlatforms() {
  try {
    const result = await db.query`SELECT * FROM freelance_platforms ORDER BY "order" ASC`
    return result as Array<{ id: number; name: string; profile_url: string; logo_url: string | null; color: string | null; enabled: boolean; order: number; created_at: string; updated_at: string }>
  } catch (error) {
    console.error('Error getting freelance platforms:', error)
    return []
  }
}

export async function createFreelancePlatform(data: { name: string; profile_url: string; logo_url?: string | null; color?: string | null; order?: number }) {
  try {
    await requireAdmin()

    const result = await db.query`
      INSERT INTO freelance_platforms (name, profile_url, logo_url, color, "order")
      VALUES (${data.name}, ${data.profile_url}, ${data.logo_url ?? null}, ${data.color ?? null}, ${data.order ?? 0})
      RETURNING *
    `
    ...
    console.error('Error creating freelance platform:', error)
    return { error: error instanceof Error ? error.message : 'Failed to create platform' }
  }
}

export async function updateFreelancePlatform(id: number, data: { name?: string; profile_url?: string; logo_url?: string | null; color?: string | null; enabled?: boolean; order?: number }) {
  try {
    await requireAdmin()

    const result = await db.query`
      UPDATE freelance_platforms
      SET
        name = COALESCE(${data.name ?? null}, name),
        profile_url = COALESCE(${data.profile_url ?? null}, profile_url),
        logo_url = COALESCE(${data.logo_url ?? null}, logo_url),
        color = COALESCE(${data.color ?? null}, color),
        enabled = COALESCE(${data.enabled ?? null}, enabled),
        "order" = COALESCE(${data.order ?? null}, "order"),
    ...
    console.error('Error updating freelance platform:', error)
    return { error: error instanceof Error ? error.message : 'Failed to update platform' }
  }
}

export async function deleteFreelancePlatform(id: number) {
  try {
    await requireAdmin()
    await db.query`DELETE FROM freelance_platforms WHERE id = ${id}`

    revalidatePath('/admin/services')
    revalidatePublic()
    return { success: true }
  } catch (error) {
    console.error('Error deleting freelance platform:', error)
...
export async function getSiteToggles() {
  try {
    const result = await db.query`SELECT * FROM site_toggles ORDER BY key ASC`
    const toggles: Record<string, boolean> = {}
    result.forEach((row: any) => {
      toggles[row.key] = Boolean(row.value)
    })
    return toggles
  } catch (error) {
    console.error('Error getting site toggles:', error)
    return {}
  }
}

export async function getSiteToggle(key: string): Promise<boolean> {
  try {
    const result = await db.query`SELECT value FROM site_toggles WHERE key = ${key} LIMIT 1`
    return result.length > 0 ? Boolean(result[0].value) : true
  } catch (error) {
    console.error('Error getting site toggle:', error)
    return true
  }
}

export async function updateSiteToggle(key: string, value: boolean) {
  try {
    await requireAdmin()
    ...
    revalidatePath('/admin/settings-toggles')
    revalidatePublic()
    return { success: true, data: result[0] }
  } catch (error) {
    console.error('Error updating site toggle:', error)
    return { error: error instanceof Error ? error.message : 'Failed to update toggle' }
  }
}
