...
export default function EducationAdminPage() {
  ...
  function startEdit(x: Education) {
    setEditingId(x.id)
    setForm({
      year: x.year ?? '',
      degree: x.degree ?? '',
  ...
  async function save() {
    try {
      ...
      if (!form.year.trim() || !form.degree.trim() || !form.institution.trim() || !form.details.trim()) {
        setError('Year, Degree, Institution, Details are required.')
        return
      }
      ...
      const res = editingId
        ? await updateEducation(editingId, payload as any)
                ...
                <div key={x.id} className="p-4 bg-slate-900/40 rounded-lg flex items-start justify-between gap-4">
                  <div>
                    <p className="text-white font-semibold">
                      {x.year} — {x.degree} @ {x.institution} {x.enabled ? '' : <span className="text-slate-400">(disabled)</span>}
                    </p>
