...
import { getEducations } from "@/app/actions/cms"
...
export default async function AboutEducationAdminPage() {
  await requireAdmin()
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {items.length ? (
            ...
            {items.map((x) => (
              <div key={x.id} className="bg-slate-800 border border-slate-700 rounded-lg p-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-white font-semibold">{x.degree}</p>
                    <p className="text-slate-400 text-sm">{x.institution} • {x.year}</p>
                  </div>
          ...
          <p className="text-slate-400 mt-4">
            No items yet in <code className="text-slate-200">educations</code>.
          </p>
        )}
      </div>
    </div>
